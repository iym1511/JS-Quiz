...
const Quiz54 = () => {
  ...
  const stamp = prompt('숫자 스탬프를 입력하세요').split(' ');

  const func = (e) => {
    let result = e.slice();
    let resultSort = result.sort((a,b) => a - b);

    if(JSON.stringify(resultSort) === JSON.stringify(e)){
      console.log('yes')
    }else{
      console.log('no')
    }
